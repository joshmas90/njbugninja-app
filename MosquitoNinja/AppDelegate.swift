...
private final class NinjaLaunchOverlay: UIView {
    ...
    private func animateMark(){mark.alpha=1;let a=CAKeyframeAnimation(keyPath:"transform");a.values=[CATransform3DConcat(CATransform3DMakeScale(0.46,0.46,1),CATransform3DMakeRotation(-42 * .pi / 180,0,0,1)),CATransform3DConcat(CATransform3DMakeScale(1.07,1.07,1),CATransform3DMakeRotation(3 * .pi / 180,0,0,1)),CATransform3DIdentity];a.keyTimes=[0,0.48,1];a.duration=1.32;a.beginTime=CACurrentMediaTime()+0.12;a.fillMode = .backwards;a.timingFunction=CAMediaTimingFunction(controlPoints:0.14,0.76,0.18,1);mark.layer.add(a,forKey:"markEntrance");animateOpacity(mark.layer,values:[0,1,1],keyTimes:[0,0.48,1],duration:1.32,delay:0.12)}
    private func animateStrike(){let begin=CACurrentMediaTime()+1.02;[strikeGlow,strike].forEach{l in l.strokeEnd=1;let d=CABasicAnimation(keyPath:"strokeEnd");d.fromValue=0;d.toValue=1;d.duration=0.72;d.beginTime=begin;d.fillMode = .backwards;l.add(d,forKey:"strikeDraw");let f=CAKeyframeAnimation(keyPath:"opacity");f.values=[0,1,1,0];f.keyTimes=[0,0.12,0.72,1];f.duration=0.72;f.beginTime=begin;l.opacity=0;l.add(f,forKey:"strikeFade")};animateScale(strikeTip,values:[0.2,1,1.7],keyTimes:[0,0.35,1],duration:0.60,delay:1.08);animateOpacity(strikeTip,values:[0,1,0],keyTimes:[0,0.35,1],duration:0.60,delay:1.08)}
    private func animateWordmark(){wordmark.alpha=1;let a=CAKeyframeAnimation(keyPath:"transform");a.values=[CATransform3DConcat(CATransform3DMakeTranslation(0,20,0),CATransform3DMakeScale(0.90,0.90,1)),CATransform3DIdentity];a.keyTimes=[0,1];a.duration=1.02;a.beginTime=CACurrentMediaTime()+1.26;a.fillMode = .backwards;a.timingFunction=CAMediaTimingFunction(controlPoints:0.16,0.82,0.2,1);wordmark.layer.add(a,forKey:"wordmarkEntrance");animateOpacity(wordmark.layer,values:[0,1,1],keyTimes:[0,0.48,1],duration:1.02,delay:1.26);let start=CGRect(x:wordmark.bounds.midX-1,y:0,width:2,height:wordmark.bounds.height);let r=CABasicAnimation(keyPath:"path");r.fromValue=UIBezierPath(rect:start).cgPath;r.toValue=UIBezierPath(rect:wordmark.bounds).cgPath;r.duration=1.02;r.beginTime=CACurrentMediaTime()+1.26;r.fillMode = .backwards;wordmarkMask.add(r,forKey:"wordmarkReveal")}
    ...
    private func animateRotation(_ l:CALayer,from:CGFloat,to:CGFloat,duration:CFTimeInterval,delay:CFTimeInterval){let a=CABasicAnimation(keyPath:"transform.rotation.z");a.fromValue=from;a.toValue=to;a.duration=duration;a.beginTime=CACurrentMediaTime()+delay;a.fillMode = .backwards;a.timingFunction=CAMediaTimingFunction(controlPoints:0.16,0.78,0.18,1);l.add(a,forKey:"rotation-\(delay)")}
}
