...
final class AppointmentsViewController: NinjaBaseViewController {
    ...
    private func buildUI() {
        ...
        if let next = AppointmentStore.shared.nextAppointment {
            ...
            contentStack.addArrangedSubview(primaryButton("Add Confirmed Appointment", symbol: "calendar.badge.plus", action: #selector(addAppointment)))
        }
        ...
        if !future.isEmpty {
            ...
            for appointment in future {
                contentStack.addArrangedSubview(compactAppointmentCard(appointment))
            }
        }
        ...
        contentStack.addArrangedSubview(card(
            ...
            detail: "Appointment reminders are scheduled on your iPhone. No live location sharing is used, requested, or needed.",
    ...
    private func appointmentHero(_ appointment: ServiceAppointment) -> UIView {
        ...
        status.textColor = NinjaPalette.green
        status.font = .systemFont(ofSize: 11, weight: .heavy)

        let date = UILabel()
        date.text = dateFormatter.string(from: appointment.startDate).uppercased()
        date.textColor = .white
        date.font = .systemFont(ofSize: 20, weight: .black)
        date.numberOfLines = 0

        let time = UILabel()
        time.text = timeFormatter.string(from: appointment.startDate)
        time.textColor = NinjaPalette.red
        ...
        property.text = appointment.propertyLabel.isEmpty ? "South Jersey service appointment" : appointment.propertyLabel
        property.textColor = NinjaPalette.muted
        property.font = .systemFont(ofSize: 14)
        property.numberOfLines = 0

        let stack = UIStackView(arrangedSubviews: [status, date, time, service, property])
    ...
    private func compactAppointmentCard(_ appointment: ServiceAppointment) -> UIControl {
        ...
        date.textColor = .white
        date.font = .systemFont(ofSize: 14, weight: .bold)

        let detail = UILabel()
        detail.text = "\(timeFormatter.string(from: appointment.startDate))  •  \(appointment.service.shortName)"
        detail.textColor = NinjaPalette.muted
        detail.font = .systemFont(ofSize: 13)

        let labels = UIStackView(arrangedSubviews: [date, detail])
        ...
        NSLayoutConstraint.activate([
            row.topAnchor.constraint(equalTo: control.topAnchor, constant: 15),
            ...
            row.bottomAnchor.constraint(equalTo: control.bottomAnchor, constant: -15)
        ])
        control.addAction(UIAction { [weak self] _ in
            self?.navigationController?.pushViewController(AppointmentEditorViewController(appointment: appointment), animated: true)
        }, for: .touchUpInside)
        return control
    }

    private func emptyAppointmentCard() -> UIView {
        card(
            title: "No appointment saved",
            detail: "Once your service time is confirmed, add it here to keep the date, preparation guide, and reminders together.",
            symbol: "calendar",
    ...
    @objc private func addAppointment() {
        navigationController?.pushViewController(AppointmentEditorViewController(), animated: true)
    }

    @objc private func editNext() {
        guard let appointment = AppointmentStore.shared.nextAppointment else { return }
        navigationController?.pushViewController(AppointmentEditorViewController(appointment: appointment), animated: true)
    }
